 ...
 */
public class ManualAskConsumerA {
    public static void main(String[] args) {
        final String className = ManualAskConsumerA.class.getSimpleName();
        // 获取 channel
        try {
            final Channel channel = RabbitMqUtils.getChannel();
            System.out.println("等待接收消息处理时间较短的消费者：" + className);
            // 采用手动应答的方式处理消息
            // 声明消息接收成功后的回调处理
            ...
            // 设置不公平分发模式，即能者多劳，可有效利用服务器性能
            // 默认为0，即采用轮询分发模式
            // 当 prefetchCount 大于 1 时，表示采用预取值模式，即代表该消费者会收到 prefetchCount 条消息，不论处理速度如何。
            // 这里设置预取值为 2，表示该消费者会接收到 2 条消息，也可能由于消息处理速度的原因，接收到的消息数量与预取值不一致。
            // 但该消费者未接收的消息个数不会超过 2 个
